...
module.exports = app => {
  const { client, clients, url, options, defaultDB, customPromise, loadModel, plugins, modelDir, modelName } = app.config.mongoose;
  ...
  if (loadModel) {
    app.beforeStart(() => {
      loadModelToApp(app, modelDir, modelName);
    });
  }
};
...
function loadModelToApp(app, modelDir, modelName) {
  const dir = path.join(app.config.baseDir, 'app/' + modelDir);
  app.loader.loadToApp(dir, modelName, {
    inject: app,
    caseStyle: 'upper',
    filter(model) {
